...
train_dataset = train_dataset.map(
    label_encoder.encode_batch, num_parallel_calls=tf.data.AUTOTUNE 
)
...
epochs = 2
# Running 200 training and 100 validation steps,
...
image = tf.keras.Input(shape=[None, None, None], name="image")
...
"""
for detect in range(50):
    take()
    def prepare_image_edit(image):
    ...
    for image in val_dataset.take(1): # !!!! NEVER CHANGE THIS!!!!
        image = tf.keras.preprocessing.image.load_img('img.jpg') #img for detecting location 
